...
import com.acatapps.videomaker.databinding.LayoutChangeDurationToolsBinding
import com.acatapps.videomaker.databinding.LayoutChangeFilterToolsBinding
import com.acatapps.videomaker.databinding.LayoutChangeThemeToolsBinding
import com.acatapps.videomaker.databinding.LayoutChangeTransitionToolsBinding
import java.io.File
...
class ImageSlideShowActivity : BaseSlideShow() {
    ...
    override fun doInitViews() {
        ...
        toolsBinding.changeEffectTools.visibility = View.GONE
        toolsBinding.changeTrimsTools.visibility = View.GONE
        needShowDialog = true
    ...
    override fun doInitActions() {
        ...
        toolsBinding.videoControllerView.onChangeListener = object :VideoControllerView.OnChangeListener {
        ...
        toolsBinding.changeThemeTools.setOnClickListener {
            if (toolType == ToolType.THEME || !mTouchEnable) return@setOnClickListener
            toolType = ToolType.THEME
            showLayoutChangeTheme()
        }

        toolsBinding.changeTransitionTools.setOnClickListener {
    ...
    private fun showLayoutChangeTheme() {
        val themeViewBinding = LayoutChangeThemeToolsBinding.inflate(layoutInflater)
        showToolsActionLayout(themeViewBinding.root)

        themeViewBinding.imageOfSlideShowListViewInChangeTheme.layoutManager =
            LinearLayoutManager(this, LinearLayoutManager.HORIZONTAL, false)
        themeViewBinding.imageOfSlideShowListViewInChangeTheme.adapter = mSlideSourceAdapter

        themeViewBinding.themeListView.adapter = mNewThemeListAdapter
        themeViewBinding.themeListView.layoutManager = LinearLayoutManager(this, LinearLayoutManager.HORIZONTAL, false)
        ...
        themeViewBinding.icAddPhotoInChangeTheme.setOnClickListener {
           doAddMoreImage()
        }
    ...
    private fun showLayoutChangeTransition() {
        val transitionViewBinding = LayoutChangeTransitionToolsBinding.inflate(layoutInflater)
        showToolsActionLayout(transitionViewBinding.root)

        transitionViewBinding.imageOfSlideShowListViewInChangeTransition.layoutManager =
            LinearLayoutManager(this, LinearLayoutManager.HORIZONTAL, false)
        transitionViewBinding.imageOfSlideShowListViewInChangeTransition.adapter = mSlideSourceAdapter
        transitionViewBinding.gsTransitionListView.adapter = mGSTransitionListAdapter
        transitionViewBinding.gsTransitionListView.layoutManager = LinearLayoutManager(this, LinearLayoutManager.HORIZONTAL, false)
        mGSTransitionListAdapter.highlightItem(mGsTransition)

        transitionViewBinding.icAddPhotoInChangeTransition.setOnClickListener {
           doAddMoreImage()
        }
    }
    ...
    private fun showLayoutChangeDuration() {
        val durationViewBinding = LayoutChangeDurationToolsBinding.inflate(layoutInflater)
        showToolsActionLayout(durationViewBinding.root)
        val totalTimeMs = (mImageSlideDataContainer.getCurrentDelayTimeMs()+mImageSlideDataContainer.transitionTimeMs)
        durationViewBinding.changeDurationSeekBar.setCurrentDuration(totalTimeMs/1000)
        durationViewBinding.totalDurationLabel.text = Utils.convertSecToTimeString(mImageSlideDataContainer.getMaxDurationMs() / 1000)

        durationViewBinding.changeDurationSeekBar.setDurationChangeListener({
            doPauseVideo()
            doChangeDelayTime(it)
            mShouldReload = true
            toolsBinding.videoControllerView.setCurrentDuration(0)
            durationViewBinding.totalDurationLabel.text = Utils.convertSecToTimeString(mImageSlideDataContainer.getMaxDurationMs()/ 1000)
            toolsBinding.videoControllerView.setMaxDuration(mImageSlideDataContainer.getMaxDurationMs())
        }, {
            doRepeat()
    ...
    private fun showLayoutChangeFilter() {
        doPauseVideo()
        val filterViewBinding = LayoutChangeFilterToolsBinding.inflate(layoutInflater)
        showToolsActionLayout(filterViewBinding.root)

        filterViewBinding.lookupListView.adapter = mLookupListAdapter
        filterViewBinding.lookupListView.layoutManager =  LinearLayoutManager(this, LinearLayoutManager.HORIZONTAL, false)

        filterViewBinding.imageListView.adapter = mImageWithLookupAdapter.apply {
            setItemList(mImageSlideDataContainer.getSlideList())
        }
        filterViewBinding.numberImageLabel.text = "${mImageWithLookupAdapter.itemCount} ${getString(R.string.photos)}"
        filterViewBinding.imageListView.layoutManager =  LinearLayoutManager(this, LinearLayoutManager.HORIZONTAL, false)
        onStick()
    }
