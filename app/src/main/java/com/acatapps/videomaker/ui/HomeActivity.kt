...
import com.acatapps.videomaker.databinding.ActivityHomeBinding
...
class HomeActivity : BaseActivity() {
    ...
    override fun initViews() {
        mHomeBinding = ActivityHomeBinding.bind(binding.mainContentLayout.getChildAt(0))
        comebackStatus = getString(R.string.do_you_want_to_leave)
        hideHeader()

        mHomeBinding.myStudioListView.apply {
            adapter = mMyStudioAdapter
            layoutManager = LinearLayoutManager(this@HomeActivity, LinearLayoutManager.HORIZONTAL, false)
        }

        mHomeBinding.newThemeListView.apply {
    ...
    fun showAdsAndRemoveVideoView() {
        binding.baseRootView.removeView(mVideoView)
        onInit()
    }
    ...
    override fun initActions() {
        mHomeBinding.bgButtonSlideShow.setOnClickListener {
            gotoPickMedia(MediaKind.PHOTO)
        }
        mHomeBinding.bgButtonEditVideo.setOnClickListener {
            gotoPickMedia(MediaKind.VIDEO)
        }
        mHomeBinding.bgTrimVideo.setClick {
            gotoPickMedia(VideoActionKind.TRIM)
        }
        mHomeBinding.bgJoinVideo.setClick {
            gotoPickMedia(VideoActionKind.JOIN)
        }

        mHomeBinding.bgRating.setClick {
            if(mRateAvailable) {
        ...
        mHomeBinding.buttonMore.setClick {
            if(pickMediaAvailable) {
                pickMediaAvailable = false
    ...
    private fun getAllMyStudioItem() {
        Thread{
            ...
            if(folder.exists() && folder.isDirectory) {
                for(item in folder.listFiles() ?: emptyArray()) {
                    try {
            ...
            runOnUiThread {
                ...
                if(mMyStudioAdapter.itemCount < 1) {
                    mHomeBinding.icNoProject.visibility = View.VISIBLE
                    mHomeBinding.buttonMore.visibility = View.GONE
                } else {
                    mHomeBinding.icNoProject.visibility = View.GONE
                    mHomeBinding.buttonMore.visibility = View.VISIBLE
                }
            }
