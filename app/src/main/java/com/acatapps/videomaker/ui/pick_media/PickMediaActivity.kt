...
import com.acatapps.videomaker.databinding.ActivityPickMediaBinding
...
class PickMediaActivity : BaseActivity(), KodeinAware {
    ...
    private lateinit var mPickMediaContentBinding: ActivityPickMediaBinding
    ...
    override fun initViews() {
        mPickMediaContentBinding = ActivityPickMediaBinding.bind(binding.mainContentLayout.getChildAt(0))
                ...
                MediaKind.VIDEO -> {
                    ...
                    actionKind?.let {
                        ...
                        if (mVideoActionKind == VideoActionKind.TRIM) {
                            ...
                            mPickMediaContentBinding.imagePickedArea.visibility = View.GONE
                        }
        ...
        mPickMediaContentBinding.tabLayout.setupWithViewPager(mPickMediaContentBinding.viewPager)
        mPickMediaContentBinding.viewPager.offscreenPageLimit = 2
        mPickMediaContentBinding.viewPager.adapter = PickMediaPagerAdapter(this, supportFragmentManager)

        val col = (DimenUtils.screenWidth(this) / (96 * DimenUtils.density(this))).roundToInt()
        mPickMediaContentBinding.mediaPickedListView.adapter = mMediaPickedAdapter
        mPickMediaContentBinding.mediaPickedListView.layoutManager = GridLayoutManager(this, col.toInt())
        addItemTouchCallback(mPickMediaContentBinding.mediaPickedListView)
        mPickMediaContentBinding.imagePickedArea.visibility = View.GONE
        ...
        for (path in mListPhotoPath) {
             ...
             mPickMediaContentBinding.imagePickedArea.visibility = View.VISIBLE
             mPickMediaContentBinding.mediaPickedListView.scrollToPosition(mMediaPickedAdapter.itemCount - 1)
             updateNumberImageSelected()
        }
    ...
    override fun initActions() {
        ...
        mPickMediaContentBinding.expandViewButton.setOnClickListener {
            if (isExpanded) collapseView()
            else expandView()
        }

        mPickMediaContentBinding.startButton.setClick {
            if(startAvailable) {
    ...
    fun updateNumberImageSelected() {
        ...
        mPickMediaContentBinding.numberMediaPicked.text = spannable
    ...
    private fun expandView() {
        ...
        mPickMediaContentBinding.expandViewButton.rotation = 180f
        val target = DimenUtils.screenHeight(this) * 2 / 3 - 220 * DimenUtils.density(this)
        mPickMediaContentBinding.imagePickedArea.layoutParams.height += target.toInt()
        mPickMediaContentBinding.imagePickedArea.requestLayout()
        isExpanded = true
    }

    private fun collapseView() {
        if (!isExpanded) return
        mPickMediaContentBinding.expandViewButton.rotation = 0f
        val target = DimenUtils.screenHeight(this) * 2 / 3 - 220 * DimenUtils.density(this)
        mPickMediaContentBinding.imagePickedArea.layoutParams.height -= target.toInt()
        mPickMediaContentBinding.imagePickedArea.requestLayout()
        isExpanded = false
