...
import com.acatapps.videomaker.databinding.ActivityProcessVideoBinding
import java.io.File
...
class ProcessVideoActivity : BaseActivity() {
    ...
    private lateinit var mProcessContentBinding: ActivityProcessVideoBinding
    ...
    private fun showNativeAds() {
        ...
        if(ad != null) {
            Utils.bindBigNativeAds(ad, mProcessContentBinding.nativeAdViewInProcess.root)
            mProcessContentBinding.nativeAdViewInProcess.root.visibility = View.VISIBLE
        } else {
            mProcessContentBinding.nativeAdViewInProcess.root.visibility = View.GONE
        }

    }

    override fun initViews() {
        mProcessContentBinding = ActivityProcessVideoBinding.bind(binding.mainContentLayout.getChildAt(0))
        ...
        if (action == renderSlideAction) {
            bundle?.let { it ->
                ...
                Observable.fromCallable<String> {
                    ...
                    imageSlideEncode.performEncodeVideo({
                        runOnUiThread {
                            mProcessContentBinding.progressBar.setProgress(it * 100)
                        }
                    }, {outPath ->
                        runOnUiThread {
                            onComplete(outPath)
           ...
           mJoinVideoHashMap.forEach {
            ...
            Thread{
                ...
                ffmpeg.runCmd({
                    runOnUiThread {
                        mProcessContentBinding.progressBar.setProgress(it*100f/total)
                    }
                },{ runOnUiThread {
    ...
    private val onReSizeDoJoinProgress = {progress:Double ->
        ...
        runOnUiThread {
            mProcessContentBinding.progressBar.setProgress(progress.toFloat())
        }
    }
    ...
    private val onReSizeDoJoinComplete = { inPath:String, outPath:String ->
        ...
        if(mJoinProcessCount == mUnDuplicateFilePathList.size) {
            Logger.e("doneee!!")
            runOnUiThread {
                mProcessContentBinding.progressBar.setProgress(90f)
            }
            joinReSizeVideo()
        } else {
            doReSizeForJoinVideo()
    ...
    override fun initActions() {
        mProcessContentBinding.cancelButton.setOnClickListener {
    ...
    private fun onComplete(filePath: String) {
        if (mIsCancel) {
            ...
            if(!onPause) {
                ...
                showToast(filePath)
                runOnUiThread {
                    mProcessContentBinding.progressBar.setProgress(100f)
                }
    ...
    private var updateProgress = {progress: Double ->
        ...
        runOnUiThread {
         mProcessContentBinding.progressBar.setProgress((progress.toFloat()*mCurrentVideoDuration+mTimeOffset)*100*0.8f/mTotalVideoTime)
        }
    }
    ...
    private fun joinVideoSlide() {
        ...
        val outJoinVideoPath = joiVideoSameType(mTempVideoSlidePathList) {
            runOnUiThread {
                mProcessContentBinding.progressBar.setProgress(80f+0.1f*it)
            }
        }
