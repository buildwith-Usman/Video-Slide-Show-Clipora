...
class JoinVideoActivity2 : BaseActivity() {
    ...
    private lateinit var mJoinBinding: ActivityJoinVideoBinding
    ...
    override fun initViews() {
        mJoinBinding = ActivityJoinVideoBinding.bind(binding.mainContentLayout.getChildAt(0))

        setScreenTitle(getString(R.string.join))
        mJoinBinding.videoPlayerView.useController = false
    ...
    private fun changeVideo(path:String) {
        ...
        if(mPlayer == null) {
            ...
            mJoinBinding.videoPlayerView.player = mPlayer
    ...
    private fun listenVideoTime() {
        mTimer = object :CountDownTimer(60*60*1000, 40) {
            ...
            override fun onTick(millisUntilFinished: Long) {
                runOnUiThread {
                    mJoinBinding.videoControllerView.setCurrentDuration(mTimelineOffset+(mPlayer?.currentPosition ?: 0))
                }
            }
    ...
    override fun initActions() {
        mJoinBinding.videoControllerView.onChangeListener = object : VideoControllerView.OnChangeListener {
        ...
        mJoinBinding.buttonJoinVideo.setClick {
            doPauseVideo()
        ...
        mJoinBinding.bgView.setOnClickListener {
            if(mDoJoin) {
                doRestartVideo()
            } else {
    ...
    private fun onSelectItem(videoId:Int) {
        var time = 0
        var targetIndex = 0
        mJoinBinding.icPlay.visibility = View.GONE
        mIsPlaying = true
        for (item in mVideoDataList) {
            if(item.id == videoId) {
                mCurrentVideoIndex = targetIndex
    ...
    private fun setupData(imageList: ArrayList<String>) {
        ...
        Thread{
            ...
            runOnUiThread {
                ...
                dismissProgressDialog()
                mJoinBinding.videoControllerView.setMaxDuration(totalDuration)
            }
    ...
    private fun setupListView() {
        mJoinBinding.videoListView.apply {
    ...
    private fun doPauseVideo() {
        ...
        mTimer?.cancel()
        mJoinBinding.icPlay.visibility = View.VISIBLE
    }
