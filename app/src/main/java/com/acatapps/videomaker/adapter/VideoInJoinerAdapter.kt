...
import com.acatapps.videomaker.databinding.ItemVideoInJoinerBinding
...
class VideoInJoinerAdapter : BaseAdapter<VideoForJoinDataModel>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BaseViewHolder {
        val binding = ItemVideoInJoinerBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        return BaseViewHolder(binding)
    }

    override fun onBindViewHolder(holder: BaseViewHolder, position: Int) {
        val item = mItemList[position]
        val binding = holder.binding as ItemVideoInJoinerBinding
        Glide.with(binding.root.context).load(item.path).apply(RequestOptions().override(imageSize.toInt())).into(binding.mediaThumb)
        binding.durationLabel.text = Utils.convertSecToTimeString((MediaUtils.getVideoDuration(item.path).toFloat()/1000).roundToInt())
