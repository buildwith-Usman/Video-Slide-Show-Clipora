...
import com.acatapps.videomaker.databinding.ItemMediaWithTextCountBinding
...
class MediaListAdapter(val callback:(MediaDataModel)->Unit) : BaseAdapter<MediaDataModel>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BaseViewHolder {
        return if(viewType == R.layout.item_header_view_date) {
            val binding = ItemHeaderViewDateBinding.inflate(LayoutInflater.from(parent.context), parent, false)
            BaseViewHolder(binding)
        } else {
            val binding = ItemMediaWithTextCountBinding.inflate(LayoutInflater.from(parent.context), parent, false)
            BaseViewHolder(binding)
        }
    }
    ...
    override fun onBindViewHolder(holder: BaseViewHolder, position: Int) {
        ...
        if(getItemViewType(position) == R.layout.item_header_view_date) {
                    ...
                    if(calendar.get(Calendar.MONTH) != today.get(Calendar.MONTH)) {
                        ...
                        if(calendar.get(Calendar.DAY_OF_MONTH) != today.get(Calendar.DAY_OF_MONTH)) {
                            if(today.timeInMillis-calendar.timeInMillis < (24*60*60*1000)) {
                                binding.dateAddedLabel.text = holder.itemView.context.getString(R.string.yesterday)
                            } else {
                                binding.dateAddedLabel.text = "${calendar.get(Calendar.DAY_OF_MONTH)}/${calendar.get(Calendar.MONTH)+1}/${calendar.get(Calendar.YEAR)}"
                            }
                        } else {
                            binding.dateAddedLabel.text = holder.itemView.context.getString(R.string.today)
                        }
                    }
                }
            }
            ...
            val binding = ItemMediaWithTextCountBinding.bind(holder.itemView)
            if(activeCounter) {
                binding.mediaThumb.activeCounter()
            } else {
                binding.mediaThumb.disableCounter()
            }
            binding.mediaThumb.setCount(item.count)
            if(item.kind == MediaKind.VIDEO) {
                binding.durationLabel.apply {
            ...
            Glide.with(holder.itemView.context).load(item.filePath).placeholder(R.drawable.ic_load_thumb).apply(RequestOptions().override(mImageSize)).into(binding.mediaThumb)
