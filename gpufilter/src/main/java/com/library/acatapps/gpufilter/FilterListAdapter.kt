...
import com.library.acatapps.gpufilter.databinding.ItemFilterModuleBinding
...
class FilterListAdapter(private val context: Context) :
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val binding = ItemFilterModuleBinding.inflate(LayoutInflater.from(parent.context), parent, false)
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        if (item.selected) {
            binding.percentFilterLabel.visibility = View.VISIBLE
            binding.percentFilterLabel.text = item.currentPercent.toString()
        } else {
            binding.percentFilterLabel.visibility = View.GONE
        }
        if (!item.filterAdjuster.canAdjust()) {
            binding.percentFilterLabel.text = ""
        }

        binding.titleFilterLabel.text = item.title
        val d = Observable.fromCallable<Bitmap> {
            ...
            .subscribeBy(
                onNext = {
                    binding.demoImageFilter.setImageBitmap(it)
                },
                onError = {
    ...
    inner class ViewHolder(val binding: ItemFilterModuleBinding) : RecyclerView.ViewHolder(binding.root)
